...
SELECT @e1 := (SELECT id FROM eventos WHERE titulo='Conferencia de IA' AND fecha_inicio='2025-10-01' LIMIT 1);
...
SELECT @e2 := (SELECT id FROM eventos WHERE titulo='Jornada DevOps' AND fecha_inicio='2025-11-05' LIMIT 1);
...
SELECT @e3 := (SELECT id FROM eventos WHERE titulo='Seminario Data Science' AND fecha_inicio='2025-12-10' LIMIT 1);
...
SELECT @p1 := (SELECT id FROM ponentes WHERE nombre='Ana Pérez' LIMIT 1);
...
SELECT @p2 := (SELECT id FROM ponentes WHERE nombre='Carlos Gómez' LIMIT 1);
...
SELECT @p3 := (SELECT id FROM ponentes WHERE nombre='Lucía Martínez' LIMIT 1);
...
-- Verificaciones finales: conteos por evento y relaciones
SELECT e.id, e.titulo, (SELECT COUNT(*) FROM asistentes a WHERE a.evento_id = e.id) AS asistentes_count
FROM eventos e
WHERE e.titulo IN ('Conferencia de IA','Jornada DevOps','Seminario Data Science')
ORDER BY e.id;

SELECT ep.evento_id, ep.ponente_id, p.nombre AS ponente
FROM evento_ponente ep
JOIN ponentes p ON p.id = ep.ponente_id
WHERE ep.evento_id IN (@e1,@e2,@e3)
ORDER BY ep.evento_id;
