...
def solve_complex_inequality(expression, variable):
    """Solve complex inequalities that may have variable on both sides of division."""
    try:
        # Find the inequality operator
        operators = ['≤', '≥', '<', '>']
        operator = None
        ...
        # Handle cases where variable appears on both sides like '-48J-76'
        if variable in right_side and not any(char in right_side for char in '()*/'):
            # This is a case where we have variable terms on both sides
            return solve_variable_both_sides_complex(left_side, right_side, operator, variable)
        
        return f"Complex case not handled: {expression}"
        
    except Exception as e:
        return f"Complex error: {str(e)}"

def solve_variable_both_sides_complex(left_side, right_side, operator, variable):
    """Handle complex cases where variable appears on both sides."""
    try:
        # Handle cases like "-13(-58J-29)/-43 ≤ -48J-76"
        # First solve the left side as a parenthetical/fraction expression
        # Then move right side variable to left
        
        # Parse right side variable terms
        right_parts = right_side.split(variable)
        if len(right_parts) == 2:
            right_coeff = parse_number(right_parts[0]) if right_parts[0] else 1
            right_const = parse_number(right_parts[1]) if right_parts[1] else 0
        else:
            # No variable on right side, treat as simple case
            if '(' in left_side and ')' in left_side:
                if '/' in left_side:
                    return solve_fraction_inequality(left_side + ' ' + operator + ' ' + right_side, variable)
                else:
                    return solve_parenthetical_inequality(left_side + ' ' + operator + ' ' + right_side, variable)
            return f"Complex case: {left_side} {operator} {right_side}"
        
        # For expressions like "-13(-58J-29)/-43 ≤ -48J-76"
        # We need to solve the left side and then handle the right side variable
        # This is quite complex, so let's implement a specialized handler
        
        if '(' in left_side and ')' in left_side and '/' in left_side:
            # Parse the complex fraction with parentheses
            # Extract outer coefficient, inner expression, divisor
            paren_start = left_side.find('(')
            paren_end = left_side.find(')')
            ...
            if not after_paren.startswith('/'):
                return "No division found in complex expression"
            
            divisor_str = after_paren[1:]  # Remove '/'
            
            # Parse components
            outer_coeff_val = parse_number(outer_coeff) if outer_coeff else 1
            divisor_val = parse_number(divisor_str)
            
            # Parse inner expression
            inner_parts = inner_expr.split(variable)
            if len(inner_parts) != 2:
                return "Variable not found in inner expression"
            
            inner_coeff = parse_number(inner_parts[0]) if inner_parts[0] else 1
            inner_const = parse_number(inner_parts[1]) if inner_parts[1] else 0
            
            # The left side is: outer_coeff_val * (inner_coeff * variable + inner_const) / divisor_val
            # Simplified: (outer_coeff_val * inner_coeff * variable + outer_coeff_val * inner_const) / divisor_val
            
            left_var_coeff = (outer_coeff_val * inner_coeff) / divisor_val
            left_const_term = (outer_coeff_val * inner_const) / divisor_val
            
            # Now we have: left_var_coeff * variable + left_const_term [operator] right_coeff * variable + right_const
            # Rearrange: (left_var_coeff - right_coeff) * variable [operator] right_const - left_const_term
            
            final_coeff = left_var_coeff - right_coeff
            final_const = right_const - left_const_term
            
            if abs(final_coeff) < 1e-10:  # Essentially zero
                return "No solution (coefficients cancel)"
        ...
        return f"Complex pattern not recognized: {left_side} {operator} {right_side}"
...
def solve_inequality(expression, variable):
    ...
    if '(' in expression and ')' in expression:
        ...
        # Check if it's a complex case with variable on both sides
        operators = ['≤', '≥', '<', '>']
        for op in operators:
            if op in expression:
                ...
                if variable in left and variable in right:
                    if any(char in expression for char in '()*/'):
                        return solve_complex_inequality(expression, variable)
                    else:
...
def main():
    ...
    print("Testing inequality solver:")
    ...
    with open('alumnos_final_solutions.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    print(f"\nCreated alumnos_final_solutions.csv with {len(solutions)} student records")
